...
export function EnhancedTableSettingsPanel({ selectedElement, onUpdate, onClose }: TableSettingsPanelProps) {
  ...
  useEffect(() => {
    if (selectedElement) {
      ...
      const themeData = selectedElement.closest('[data-theme]')?.getAttribute('data-theme')
      if (themeData) {
        try {
  ...
  const handleTableUpdate = useCallback((html: string) => {
    setTableHtml(html)
  ...
  const handleDelete = () => {
    if (selectedElement) {
      // Look for parent container first
      ...
      <ScrollArea className="flex-1">
        <div className="p-4 space-y-6">
          ...
          <div className="text-xs text-muted-foreground">
            Select cells to edit their content. Use the toolbar to add/remove rows and columns or merge/split cells.
          </div>
        </div>
      </ScrollArea>
    </div>
