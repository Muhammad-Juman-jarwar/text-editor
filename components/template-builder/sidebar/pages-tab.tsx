  ...
  return (
    <div className="space-y-4 mt-0">
          ...
          {template.pages.map((page: any, index: number) => (
            <div
              ...
              className={cn(
                ...
                selectedPageId === page.id ? "bg-primary/10 border border-primary/20" : "hover:bg-muted/50",
              )}
      ...
      {selectedPage && (
        <Card>
          <CardHeader className="pb-3">
            <CardTitle className="text-sm text-foreground">Page Settings</CardTitle>
          </CardHeader>
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                ...
                <Switch
                  ...
                  onCheckedChange={(checked) => onUpdatePage(selectedPage.id, { showHeader: checked })}
                />
              </div>
              ...
              <div className="flex items-center justify-between">
                ...
                <Switch
                  ...
                  onCheckedChange={(checked) => onUpdatePage(selectedPage.id, { showFooter: checked })}
                />
              </div>

              <div className="flex items-center justify-between">
                <Label className="text-xs text-foreground">Show Page Number</Label>
                <Switch
                  checked={selectedPage.showPageNumber}
                  onCheckedChange={(checked) => onUpdatePage(selectedPage.id, { showPageNumber: checked })}
                />
              </div>
            </div>

            <div>
              <Label className="text-xs mb-2 block text-foreground">Page Background Color</Label>
              <ColorPicker
                value={selectedPage.backgroundColor || template.theme.colors.pageBackground || "#ffffff"}
                onChange={(color) => onUpdatePage(selectedPage.id, { backgroundColor: color })}
              />
              {selectedPage.backgroundColor && (
                <Button
                  ...
                  onClick={() => onUpdatePage(selectedPage.id, { backgroundColor: undefined })}
                >
